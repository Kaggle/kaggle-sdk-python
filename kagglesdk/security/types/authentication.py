...
class AuthorizationRoleScope(KaggleObject):
    ...
    @permissions.setter
    def permissions(
        self, permissions: Optional[List[Optional["AuthorizationPermissionScope"]]]
    ):
        ...
        if not all([isinstance(t, AuthorizationPermissionScope) for t in permissions]):
            raise TypeError(
                "permissions must contain only items of type AuthorizationPermissionScope"
            )
...
AuthorizationScope._fields = [
    FieldMetadata(
        "resourceId", "resource_id", "_resource_id", int, 0, PredefinedSerializer()
    ),
    FieldMetadata(
        "permission",
        "permission",
