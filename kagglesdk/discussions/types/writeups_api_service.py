...
class ApiGetResolvedWriteUpLinksResponse(KaggleObject):
    ...
    def resolved_links(
        self, resolved_links: Optional[List[Optional["ResolvedWriteUpLink"]]]
    ):
        ...
        if not all([isinstance(t, ResolvedWriteUpLink) for t in resolved_links]):
            raise TypeError(
                "resolved_links must contain only items of type ResolvedWriteUpLink"
            )
...
ApiGetResolvedWriteUpLinksRequest._fields = [
    FieldMetadata(
        "writeUpId", "write_up_id", "_write_up_id", int, 0, PredefinedSerializer()
    ),
]
