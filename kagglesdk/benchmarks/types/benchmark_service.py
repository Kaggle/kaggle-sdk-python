...
class AddTasksToBenchmarkRequest(KaggleObject):
  ...
  @property
  def task_version_ids(self) -> Optional[List[int]]:
    return self._task_version_ids

  @task_version_ids.setter
  def task_version_ids(self, task_version_ids: Optional[List[int]]):
    if task_version_ids is None:
      del self.task_version_ids
      return
    if not isinstance(task_version_ids, list):
      raise TypeError('task_version_ids must be of type list')
    if not all([isinstance(t, int) for t in task_version_ids]):
      raise TypeError('task_version_ids must contain only items of type int')
    self._task_version_ids = task_version_ids
...
AddTasksToBenchmarkRequest._fields = [
  FieldMetadata("benchmarkVersionId", "benchmark_version_id", "_benchmark_version_id", int, 0, PredefinedSerializer()),
  FieldMetadata("taskVersionIds", "task_version_ids", "_task_version_ids", int, [], ListSerializer(PredefinedSerializer())),
]
