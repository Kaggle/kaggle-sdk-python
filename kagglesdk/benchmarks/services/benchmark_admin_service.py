...
class BenchmarkAdminClient(object):
  ...
  def admin_schedule_benchmark_task_runs(self, request: AdminScheduleBenchmarkTaskRunsRequest = None) -> AdminScheduleBenchmarkTaskRunsResponse:
    r"""
    Schedules benchmark task runs for a given model version on all
    Kaggle-maintained research benchmarks where runs are missing.

    Args:
      request (AdminScheduleBenchmarkTaskRunsRequest):
        The request object; initialized to empty instance if not specified.
    """
    ...
    return self._client.call("benchmarks.BenchmarkAdminService", "AdminScheduleBenchmarkTaskRuns", request, AdminScheduleBenchmarkTaskRunsResponse)
