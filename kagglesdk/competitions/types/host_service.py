...
from kagglesdk.kernels.types.kernels_service import GithubFileInfo
...
class CreateCompetitionRequest(KaggleObject):
  ...
  Attributes:
    ...
    clone_all_submissions (bool)
      Admin-only. If cloning, copy all non-after-deadline submissions (not just
      benchmark/sandbox ones), creating a new benchmark-style team for each.
  """
  ...
  def __init__(self):
    ...
    self._clone_all_submissions = None
    self._freeze()
  ...
  @property
  def clone_all_submissions(self) -> bool:
    r"""
    Admin-only. If cloning, copy all non-after-deadline submissions (not just
    benchmark/sandbox ones), creating a new benchmark-style team for each.
    """
    return self._clone_all_submissions or False

  @clone_all_submissions.setter
  def clone_all_submissions(self, clone_all_submissions: Optional[bool]):
    if clone_all_submissions is None:
      del self.clone_all_submissions
      return
    if not isinstance(clone_all_submissions, bool):
      raise TypeError('clone_all_submissions must be of type bool')
    self._clone_all_submissions = clone_all_submissions
...
class CreateHarnessKernelRequest(KaggleObject):
  r"""
  Attributes:
    competition_id (int)
    github_source (GithubFileInfo)
      Optional GitHub source to seed the kernel from. When set, the handler
      fetches the file via the public GitHub API and uses its content as the
      initial kernel source. If the file doesn't exist, an empty kernel is
      created. Only the owner/repo/path/ref fields are read; raw_url/size are
      ignored.
  """

  def __init__(self):
    self._competition_id = 0
    self._github_source = None
    self._freeze()
  ...
  @property
  def github_source(self) -> Optional['GithubFileInfo']:
    r"""
    Optional GitHub source to seed the kernel from. When set, the handler
    fetches the file via the public GitHub API and uses its content as the
    initial kernel source. If the file doesn't exist, an empty kernel is
    ...
    ignored.
    """
    return self._github_source or None

  @github_source.setter
  def github_source(self, github_source: Optional[Optional['GithubFileInfo']]):
    if github_source is None:
      del self.github_source
      return
    if not isinstance(github_source, GithubFileInfo):
      raise TypeError('github_source must be of type GithubFileInfo')
    self._github_source = github_source


class CreateHarnessKernelResponse(KaggleObject):
  r"""
  Attributes:
    kernel_id (int)
  """
...
CreateCompetitionRequest._fields = [
  ...
  FieldMetadata("cloneAllSubmissions", "clone_all_submissions", "_clone_all_submissions", bool, None, PredefinedSerializer(), optional=True),
]
...
CreateHarnessKernelRequest._fields = [
  FieldMetadata("competitionId", "competition_id", "_competition_id", int, 0, PredefinedSerializer()),
  FieldMetadata("githubSource", "github_source", "_github_source", GithubFileInfo, None, KaggleObjectSerializer(), optional=True),
]

CreateHarnessKernelResponse._fields = [
