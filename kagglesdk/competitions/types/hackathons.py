...
class HackathonTrack(KaggleObject):
    ...
    def prizes(self, prizes: Optional[List[Optional["HackathonTrackPrize"]]]):
        ...
        if not all([isinstance(t, HackathonTrackPrize) for t in prizes]):
            raise TypeError(
                "prizes must contain only items of type HackathonTrackPrize"
            )
        self._prizes = prizes
...
class HackathonWriteUp(KaggleObject):
    ...
    def awarded_hackathon_track_prize_ids(
        self, awarded_hackathon_track_prize_ids: Optional[List[int]]
    ):
        ...
        if not all([isinstance(t, int) for t in awarded_hackathon_track_prize_ids]):
            raise TypeError(
                "awarded_hackathon_track_prize_ids must contain only items of type int"
            )
...
HackathonTrack._fields = [
    ...
    FieldMetadata(
        "orderIndex", "order_index", "_order_index", float, 0.0, PredefinedSerializer()
    ),
]
...
HackathonTrackPrize._fields = [
    ...
    FieldMetadata(
        ...
        "type",
        "_type",
        HackathonTrackPrizeType,
        HackathonTrackPrizeType.HACKATHON_TRACK_PRIZE_TYPE_UNSPECIFIED,
        EnumSerializer(),
    ),
    FieldMetadata(
        "amountUsd", "amount_usd", "_amount_usd", int, 0, PredefinedSerializer()
    ),
    FieldMetadata(
        "description", "description", "_description", str, "", PredefinedSerializer()
    ),
...
HackathonWriteUp._fields = [
    FieldMetadata("id", "id", "_id", int, 0, PredefinedSerializer()),
    FieldMetadata(
        "team", "team", "_team", Team, None, KaggleObjectSerializer(), optional=True
    ),
    FieldMetadata(
        "writeUp", "write_up", "_write_up", WriteUp, None, KaggleObjectSerializer()
    ),
    FieldMetadata(
        "template", "template", "_template", bool, False, PredefinedSerializer()
    ),
    FieldMetadata(
        "hackathonTrackIds",
        "hackathon_track_ids",
        "_hackathon_track_ids",
        int,
        [],
        ListSerializer(PredefinedSerializer()),
    ),
    FieldMetadata(
        "awardedHackathonTrackPrizeIds",
        "awarded_hackathon_track_prize_ids",
        "_awarded_hackathon_track_prize_ids",
        int,
        [],
        ListSerializer(PredefinedSerializer()),
    ),
