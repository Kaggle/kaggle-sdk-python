...
class HostClient(object):
  ...
  def create_harness_kernel(self, request: CreateHarnessKernelRequest = None) -> CreateHarnessKernelResponse:
    r"""
    Creates a Python script kernel owned by the Competition Metric bot
    (UserId.MetricUserId) to serve as the harness notebook for a Game Arena
    competition. If github_source is set, seeds the kernel with the contents
    of that file and stamps the corresponding GithubSpec so the kernel editor
    shows it as linked to GitHub. The created kernel is NOT attached to the
    competition as a data source (that would mark it a submission notebook);
    callers are expected to wire it up via
    UpdateCompetitionSimulationSettings.HarnessKernelId.

    Args:
      request (CreateHarnessKernelRequest):
        The request object; initialized to empty instance if not specified.
    """
    ...
    return self._client.call("competitions.HostService", "CreateHarnessKernel", request, CreateHarnessKernelResponse)
