...
class Organization(KaggleObject):
    ...
    def featured_members(
        self, featured_members: Optional[List[Optional["UserAvatar"]]]
    ):
        ...
        if not all([isinstance(t, UserAvatar) for t in featured_members]):
            raise TypeError(
                "featured_members must contain only items of type UserAvatar"
            )
    ...
    def membership_type(self) -> "OrganizationMembershipType":
        """Membership type for the current user for this organization"""
        return (
            self._membership_type
            or OrganizationMembershipType.ORGANIZATION_MEMBERSHIP_TYPE_UNSPECIFIED
        )
    ...
    def membership_type(self, membership_type: Optional["OrganizationMembershipType"]):
        ...
        if not isinstance(membership_type, OrganizationMembershipType):
            raise TypeError(
                "membership_type must be of type OrganizationMembershipType"
            )
...
Organization._fields = [
    ...
    FieldMetadata(
        "memberCount", "member_count", "_member_count", int, 0, PredefinedSerializer()
    ),
    FieldMetadata(
        "datasetCount",
        "dataset_count",
    ...
    FieldMetadata(
        "modelCount", "model_count", "_model_count", int, 0, PredefinedSerializer()
    ),
    FieldMetadata(
        "inviteCode",
        "invite_code",
