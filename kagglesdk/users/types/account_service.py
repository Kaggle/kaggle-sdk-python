...
class GenerateAccessTokenRequest(KaggleObject):
    ...
    def authorization_context(
        ...
        if not isinstance(authorization_context, AuthorizationContext):
            raise TypeError(
                "authorization_context must be of type AuthorizationContext"
            )
    ...
    def authorization_scopes(
        self, authorization_scopes: Optional[List[Optional["AuthorizationScope"]]]
    ):
        ...
        if not all([isinstance(t, AuthorizationScope) for t in authorization_scopes]):
            raise TypeError(
                "authorization_scopes must contain only items of type AuthorizationScope"
            )
...
ExpireApiTokenRequest._fields = [
    ...
    FieldMetadata(
        "reason", "reason", "_reason", str, None, PredefinedSerializer(), optional=True
    ),
]
...
GenerateAccessTokenResponse._fields = [
    ...
    FieldMetadata(
        "expiresIn", "expires_in", "_expires_in", int, 0, PredefinedSerializer()
    ),
    FieldMetadata(
